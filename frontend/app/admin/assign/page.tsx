...
//   HardDrive, Link, AlertCircle, Folder, ChevronRight,
...
//   const [resolvedShareId, setResolvedShareId] = useState<{ driveId: string; itemId: string } | null>(null);
...
//     // Reset folder browser when URL changes
//     setRootFolders([]);
//     setSelectedFolder(null);
//     setExpandedIds(new Set());
//     setResolvedShareId(null);
//     setFetchError("");
//   }

//   // Encode share URL → Graph shareId
//   function encodeShareId(url: string) {
//     const b64 = btoa("u!" + url).replace(/=/g, "").replace(/\+/g, "-").replace(/\//g, "_");
//     return b64;
//   }
...
//     try {
//       const shareId = encodeShareId(oneDriveUrl.trim());
//       const rootRes = await fetch(
//         `https://graph.microsoft.com/v1.0/shares/${shareId}/driveItem?$select=id,name,folder,parentReference`
//       );
//       if (!rootRes.ok) {
//         const e = await rootRes.json().catch(() => ({}));
//         throw new Error(e?.error?.message || `Could not access link (${rootRes.status}). Make sure it's set to "Anyone with the link can view".`);
//       }
//       const rootItem = await rootRes.json();
//       if (!rootItem.folder) throw new Error("The shared link must point to a folder.");

//       const driveId = rootItem.parentReference?.driveId;
//       const itemId = rootItem.id;
//       setResolvedShareId({ driveId, itemId });

//       // Load immediate children (folders only)
//       const childRes = await fetch(
//         `https://graph.microsoft.com/v1.0/drives/${driveId}/items/${itemId}/children?$select=id,name,folder&$filter=folder ne null`
//       );
//       const childData = await childRes.json();
//       const folders: DriveFolder[] = (childData.value || [])
//         .filter((i: any) => i.folder)
//         .map((i: any) => ({ id: i.id, name: i.name, childCount: i.folder?.childCount ?? 0 }));

//       // If root itself has files (no subfolders shown), treat root as selectable
//       setRootFolders(folders.length > 0 ? folders : [{ id: itemId, name: rootItem.name, childCount: 0 }]);
//     } catch (e: any) {
//       setFetchError(e.message || "Failed to load OneDrive folder");
//     } finally {
...
//     if (!folder.loaded && (folder.childCount ?? 0) > 0) {
//       try {
//         const res = await fetch(
//           `https://graph.microsoft.com/v1.0/drives/${resolvedShareId.driveId}/items/${folder.id}/children?$select=id,name,folder&$filter=folder ne null`
//         );
//         const data = await res.json();
//         const children: DriveFolder[] = (data.value || [])
//           .filter((i: any) => i.folder)
//           .map((i: any) => ({ id: i.id, name: i.name, childCount: i.folder?.childCount ?? 0 }));

//         function patchTree(nodes: DriveFolder[]): DriveFolder[] {
//           return nodes.map(n =>
//             n.id === folder.id ? { ...n, children, loaded: true } : { ...n, children: n.children ? patchTree(n.children) : undefined }
//           );
//         }
//         setRootFolders(prev => patchTree(prev));
//       } catch {}
//     }
//   }

//   function renderFolderTree(folders: DriveFolder[], depth = 0): React.ReactNode {
//     return folders.map(f => {
//       const isExpanded = expandedIds.has(f.id);
//       const isSelected = selectedFolder?.id === f.id;
//       const hasChildren = (f.childCount ?? 0) > 0;

//       return (
//         <div key={f.id}>
//           <div
//             className="flex items-center gap-1.5 py-1.5 pr-3 rounded-lg cursor-pointer transition-all group"
//             style={{
...
//               style={{ color: isSelected ? "var(--primary)" : "#F59E0B" }}
//             />
//             <span
...
//             )}
//           </div>
//           {isExpanded && f.children && (
//             <div>{renderFolderTree(f.children, depth + 1)}</div>
//           )}
//         </div>
//       );
//     });
...
//     if (!form.employee_id) { setError("Please select an employee"); return; }
//     if (sourceMode === "onedrive") {
//       if (!oneDriveUrl.trim()) { setError("Please paste a OneDrive URL"); return; }
//       if (!selectedFolder) { setError("Please select a folder from the list"); return; }
//       if (!resolvedShareId) { setError("Please load the folder list first"); return; }
//     } else {
...
//       if (sourceMode === "onedrive" && selectedFolder && resolvedShareId) {
//         fd.append("onedrive_drive_id", resolvedShareId.driveId);
//         fd.append("onedrive_item_id", selectedFolder.id);
//         fd.append("onedrive_folder_name", selectedFolder.name);
//         fd.append("onedrive_web_url", oneDriveUrl.trim());
...
//   const showFolderPanel = sourceMode === "onedrive" && rootFolders.length > 0;

//   return (
//     <AdminLayout>
//       <div className="p-8" style={{ maxWidth: showFolderPanel ? "100%" : "768px" }}>
//         <div className="mb-8">
//           <h1 className="text-2xl font-bold">Assign Task</h1>
//           <p className="text-sm mt-1" style={{ color: "var(--text-muted)" }}>
//             Paste a OneDrive shared folder link or upload a ZIP, then assign to an employee
...
//         {/* Two-column when folder panel is showing */}
//         <div className={showFolderPanel ? "flex gap-6 items-start" : ""}>

//           {/* ── Left: form ─────────────────────────────────────────── */}
//           <form
//             onSubmit={handleSubmit}
//             className="space-y-6"
//             style={{ flex: showFolderPanel ? "0 0 480px" : undefined, minWidth: 0 }}
//           >
//             {/* Employee */}
//             <div className="bg-white rounded-2xl border p-6" style={{ borderColor: "var(--border)" }}>
//               <h2 className="font-semibold mb-4 flex items-center gap-2">
...
//                     style={{ background: "var(--primary-light)", color: "var(--primary-dark)" }}>
//                     {c}
//                     <button type="button" onClick={() => removeClass(c)} className="hover:text-red-500"><X size={13} /></button>
//                   </span>
//                 ))}
//               </div>
//               <div className="flex gap-2">
...
//               {/* Toggle */}
//               <div className="flex gap-2 mb-5 p-1 rounded-xl" style={{ background: "#f1f5f9" }}>
//                 <button type="button" onClick={() => setSourceMode("onedrive")}
//                   className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-medium transition-all ${
//                     sourceMode === "onedrive" ? "bg-white shadow-sm text-slate-800" : "text-slate-500"}`}>
//                   <Cloud size={15} className="text-blue-500" /> OneDrive Link
//                 </button>
//                 <button type="button" onClick={() => setSourceMode("upload")}
//                   className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-medium transition-all ${
//                     sourceMode === "upload" ? "bg-white shadow-sm text-slate-800" : "text-slate-500"}`}>
//                   <HardDrive size={15} className="text-slate-500" /> Upload ZIP
//                 </button>
//               </div>
...
//                     <button type="button"
//                       onClick={loadFolders}
//                       disabled={!oneDriveUrl || !!urlError || fetchingFolders}
//                       className="flex items-center gap-1.5 px-4 py-2.5 rounded-xl text-sm font-medium text-white disabled:opacity-50 flex-shrink-0"
//                       style={{ background: "var(--primary)" }}>
//                       {fetchingFolders
//                         ? <><RefreshCw size={14} className="animate-spin" /> Loading...</>
//                         : <><FolderOpen size={14} /> Load Folders</>}
//                     </button>
//                   </div>
...
//                   )}
//                   {fetchError && (
//                     <p className="text-xs text-red-600 bg-red-50 px-3 py-2 rounded-lg flex items-start gap-1.5">
//                       <AlertCircle size={12} className="mt-0.5 flex-shrink-0" /> {fetchError}
//                     </p>
//                   )}

//                   {selectedFolder && (
//                     <div className="flex items-center gap-2 text-xs bg-green-50 text-green-700 px-3 py-2.5 rounded-xl font-medium">
//                       <CheckCircle size={13} />
//                       Selected: <strong>{selectedFolder.name}</strong> — will be zipped and sent
//                     </div>
//                   )}
...
//           {/* ── Right: folder browser panel ─────────────────────────── */}
//           {showFolderPanel && (
//             <div
//               className="flex-1 bg-white rounded-2xl border overflow-hidden sticky top-6"
//               style={{ borderColor: "var(--border)", minWidth: 0, maxHeight: "calc(100vh - 120px)" }}
//             >
//               {/* Panel header */}
//               <div
...
//               >
//                 <div>
//                   <h2 className="font-semibold text-sm">OneDrive Folders</h2>
//                   <p className="text-xs text-slate-500 mt-0.5">
//                     {selectedFolder
//                       ? <>Selected: <span className="font-medium text-amber-700">{selectedFolder.name}</span></>
...
//                   title="Refresh"
//                 >
//                   <RefreshCw size={15} className={fetchingFolders ? "animate-spin" : ""} />
//                 </button>
//               </div>
...
//                     <RefreshCw size={16} className="animate-spin" /> Loading folders...
//                   </div>
...
//   function encodeShareId(url: string) {
//     // btoa → base64url (no padding, + → -, / → _)
//     const b64 = btoa(unescape(encodeURIComponent("u!" + url)));
//     return b64.replace(/=/g, "").replace(/\+/g, "-").replace(/\//g, "_");
//   }

//   // Load children of a node from Graph API
//   async function fetchChildren(driveId: string, itemId: string): Promise<DriveNode[]> {
//     const url = `https://graph.microsoft.com/v1.0/drives/${driveId}/items/${itemId}/children?$select=id,name,file,folder`;
...
//     }
//     const data = await res.json();
//     const nodes: DriveNode[] = (data.value || []).map((item: any) => ({
//       id: item.id,
//       name: item.name,
...
//         throw new Error(
//           e?.error?.message ||
//           `Cannot access this link (HTTP ${rootRes.status}). ` +
//           `Make sure it's set to "Anyone with the link can view".`
//         );
//       }
//       const rootItem = await rootRes.json();
//       if (!rootItem.folder) throw new Error("The shared link must point to a folder, not a file.");

//       const driveId: string = rootItem.parentReference?.driveId;
//       const rootItemId: string = rootItem.id;
//       setResolvedDrive({ driveId, rootItemId });

//       // Load first level
//       const children = await fetchChildren(driveId, rootItemId);
//       setDriveTree(children);

//       // Auto-expand if there's only one folder at root
//       const onlyFolders = children.filter(c => c.type === "folder");
//       if (onlyFolders.length === 1) {
//         setExpandedIds(new Set([onlyFolders[0].id]));
//         // pre-load its children too
//         loadNodeChildren(onlyFolders[0], driveId, children);
//       }
//     } catch (e: any) {
//       setFetchError(e.message || "Failed to load OneDrive contents");
//     } finally {
//       setFetchingFolders(false);
//     }
//   }
...
//     node: DriveNode,
//     driveId: string,
//     currentTree: DriveNode[]
//   ) {
//     if (node.type !== "folder" || node.children !== undefined) return;
//     setLoadingNodeId(node.id);
//     try {
//       const children = await fetchChildren(driveId, node.id);
...
//   async function handleToggleExpand(node: DriveNode) {
...
//         await loadNodeChildren(node, resolvedDrive.driveId, driveTree);
//       }
//     }
//     setExpandedIds(next);
//   }
...
//   // ── render tree node ──────────────────────────────────────────────────────
//   function TreeNode({ node, depth }: { node: DriveNode; depth: number }) {
...
//           onClick={() => {
//             if (isFolder) {
//               setSelectedFolder(isSelected ? null : node);
...
//           {/* Selected checkmark */}
//           {isSelected && (
//             <Check size={12} className="flex-shrink-0" style={{ color: "var(--primary)" }} />
//           )}
//         </div>
...
//         {/* Loading placeholder while fetching children */}
//         {isFolder && isExpanded && node.children === undefined && !isLoadingThis && (
//           <div
...
//     if (sourceMode === "onedrive") {
//       if (!oneDriveUrl.trim()) { setError("Please paste a OneDrive URL"); return; }
//       if (!selectedFolder) { setError("Please select a folder from the panel on the right"); return; }
//       if (!resolvedDrive) { setError("Please load the folder list first"); return; }
//     } else {
//       if (!file) { setError("Please select a ZIP file"); return; }
//     }
...
//       if (sourceMode === "onedrive" && selectedFolder && resolvedDrive) {
...
//   const showPanel = sourceMode === "onedrive";
...
//             onSubmit={handleSubmit}
//             className="space-y-6"
//             style={{ flex: "0 0 460px", minWidth: 0 }}
//           >
//             {/* Employee */}
//             <div className="bg-white rounded-2xl border p-6" style={{ borderColor: "var(--border)" }}>
//               <h2 className="font-semibold mb-4 flex items-center gap-2">
...
//                   className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-medium transition-all ${
//                     sourceMode === "onedrive" ? "bg-white shadow-sm text-slate-800" : "text-slate-500"}`}>
//                   <Cloud size={15} className="text-blue-500" /> OneDrive
//                 </button>
//                 <button type="button" onClick={() => setSourceMode("upload")}
//                   className={`flex-1 flex items-center justify-center gap-2 py-2 rounded-lg text-sm font-medium transition-all ${
//                     sourceMode === "upload" ? "bg-white shadow-sm text-slate-800" : "text-slate-500"}`}>
...
//                       <div className="flex-1 min-w-0">
//                         <p className="text-xs font-semibold text-green-800 truncate">{selectedFolder.name}</p>
...
//                       ? selectedFolder
//                         ? <>Selected: <span className="font-medium text-amber-700">{selectedFolder.name}</span></>
//                         : "Click a folder to select it for the task"
//                       : "Paste a link and click Load to browse"}
//                   </p>
//                 </div>
//                 {driveTree.length > 0 && (
//                   <button type="button" onClick={loadFolders} disabled={fetchingFolders}
//                     className="p-1.5 rounded-lg hover:bg-slate-100 text-slate-400 hover:text-slate-600 transition-colors"
//                     title="Refresh">
//                     <RefreshCw size={14} className={fetchingFolders ? "animate-spin" : ""} />
//                   </button>
//                 )}
//               </div>
...
//                       Paste a OneDrive shared folder link on the left and click <strong>Load</strong>
//                     </p>
//                   </div>
//                 ) : (
//                   driveTree.map(node => (
